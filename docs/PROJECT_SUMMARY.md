...
**Status:** ✅ Deployed — 5 domain services + Gateway + Eureka, backend on Render / frontend on Vercel
...
|---------|------|-----------------|----------|------------|
| **API Gateway** | 8080 | Route & track requests | N/A | Routes to all |
| **Eureka Server** | 8761 | Service discovery (local/Docker) | N/A | N/A |
| **User Service** | 8081 | User management | main_db | None |
| **Post Service** | 8082 | Blog posts | main_db | User + Tag |
| **Comment Service** | 8083 | Comments | main_db | User + Post |
| **Like-Dislike Service** | 8084 | Post reactions | main_db | User + Post |
| **Tag Service** | 8085 | Tags + post assignment | main_db | Feign + Resilience4j |

> All services share a single PostgreSQL database (`main_db`); each owns its own tables.
...
- **Shared Database** - Single PostgreSQL `main_db`; each service owns its own tables
...
All tables live in the single shared PostgreSQL database `main_db` (schema is auto-generated by Hibernate; shown here for reference).
...
```bash
# Easiest: run the whole stack (PostgreSQL, Kafka, Eureka, gateway, all services)
docker-compose up
...
cd eureka-server && mvn spring-boot:run          # Eureka (8761)
cd api-gateway && mvn spring-boot:run            # Gateway (8080)
cd user-service && mvn spring-boot:run           # 8081
cd post-service && mvn spring-boot:run           # 8082
cd comment-service && mvn spring-boot:run        # 8083
cd like-dislike-service && mvn spring-boot:run   # 8084
cd tag-service && mvn spring-boot:run            # 8085
...
1. **Microservices Pattern** - Independent services sharing one database, each owning its tables
2. **API Composition** - Combining data from multiple services (Post with User)
3. **Shared Database, Private Tables** - Each service owns its own tables in `main_db`
...
| Metric | Value |
|--------|-------|
| Services | 5 domain (User, Post, Comment, Like-Dislike, Tag) + Gateway + Eureka |
| REST Endpoints | 30+ across services |
| API Gateway Routes | users, posts, comments, likedislike, tags, tracking |
...
- ✅ Timeout handling
- 🔜 Retry / bulkhead tuning

**Phase 5 - Async Communication (Scaffolded):**
- 🔜 Apache Kafka wired into business events (Kafka/Zookeeper run via Docker Compose)
- 🔜 Event-driven architecture
- 🔜 Message publishing and consuming

**Phase 6 - Caching & Performance (Future):**
- Redis caching
- Database query optimization
- API response caching

**Phase 7 - ✅ Containerization (COMPLETE):**
- ✅ Docker for each service (multi-stage builds)
