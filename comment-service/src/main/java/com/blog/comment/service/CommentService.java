...
public class CommentService {
  ...
  public CommentDTO createComment(CommentDTO commentDTO) {
    log.info(
        "Creating comment on postId: {} by userId: {}",
        ...
        .map(
            comment -> {
              try {
                ...
                log.warn(
                    "Failed to load user/post for comment {}: {}", comment.getId(), e.getMessage());
  ...
  private CommentDTO mapToDTO(
      Comment comment, com.blog.comment.dto.UserDTO user, com.blog.comment.dto.PostDTO post) {
